...
const getValidProblems = (res, difficulty) => {
    ...
    difficulty = difficulty.splice(2);
...
const selectRandomProblem = (validProblems) => {
    const maxNumber = validProblems.length - 1;
    const minNumber = 0;
    const selectedproblem = validProblems[Math.floor(Math.random() * (maxNumber - minNumber + 1) + minNumber)];
...
const extractSampleInput = (s) => {
    ...
    $(s).each((index,element) => {
        count++;
        if(count%2==1){
            obj.input = $(element).text();
        }else{
            obj.output = $(element).text();
